...
> 当前仓库的 `program/miniprogram/` 还是制品占位。正式编码时会在该目录下新增真实小程序工程目录（例如 `program/miniprogram/app/`），并保留现有镜像构建脚本不受影响。

建议结构（示例）：
- `program/miniprogram/app/app.json` / `app.ts` / `app.wxss`
